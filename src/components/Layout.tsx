...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
          ...
          {navItems.filter(item => item.to !== '/coach/slots' && item.to !== '/coach/members').map((item) => (
            <Link
              ...
              className={`flex flex-col items-center gap-0.5 px-3 py-1 text-xs font-medium transition-colors ${
                isActive(item.to) ? 'text-red-600' : 'text-gray-500'
              }`}
            >
      ...
      {/* Desktop sidebar nav */}
      {sidebarOpen ? (
        <nav className="hidden md:flex fixed left-0 top-14 bottom-0 w-48 bg-white border-r border-gray-200 flex-col p-4 gap-1 z-30">
          {navItems.map((item) => (
            <Link
              key={item.to}
              to={item.to}
          ...
          <button
            ...
            className="mt-auto flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-colors"
        ...
        <button
          ...
          className="hidden md:flex fixed left-0 top-14 bottom-0 z-30 w-10 items-center justify-center bg-white border-r border-gray-200 text-gray-400 hover:text-gray-700 hover:bg-gray-50 transition-colors"
...
function CalendarIcon({ active }: { active: boolean }) {
  return (
    <svg className={`w-5 h-5 ${active ? 'text-red-600' : 'text-gray-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
